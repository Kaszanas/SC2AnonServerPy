...
def get_replays(replay_directory: str):
    ...
    for replay in os.listdir(replay_directory):
        if replay.endswith(".SC2Replay"):
            absolute_filepaths.append(
                os.path.abspath(os.path.join(replay_directory, replay))
            )
...
def start_processing(
    ...
    if args_multiprocessing:
        ...
        with Pool(processes=args_agents, initializer=initialize_worker) as pool:
            pool.imap_unordered(
                process_replay, processing_arguments_iterator, args_chunksize
            )
            pool.close()
            pool.join()
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="StarCraft II replay processing tool that uses multiprocessing."
    )
    parser.add_argument(
        "--input_dir",
        type=str,
        default="./DEMOS/Input",
        help="Provide the path to the input directory that contains .SC2Replay files.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="./DEMOS/Output/",
        help="Provide the path to the output directory that will contain .pickle files.",
    )
    parser.add_argument(
        "--agents",
        type=int,
        default=22,
        help="Provide how much agents will be available in the pool for execution.",
    )
    parser.add_argument(
        "--chunksize",
        type=int,
        default=1000,
        help="Provide how much replays are to be processed at once.",
    )
    parser.add_argument(
        "--use_multiprocessing",
        type=bool,
        default=True,
        help="Set this flag to true if You would like to use multiprocessing.",
    )
    parser.add_argument(
        "--anonymize_toon",
        type=bool,
        default=True,
        help="Set this flag to true if You would like to perform toon/nickname anonymization.",
    )
    parser.add_argument(
        "--anonymize_chat",
        type=bool,
        default=True,
        help="Set this flag to true if You would like to perform chat anonymization.",
    )
    ...
    # Starting the client:
    start_processing(
        args_replay_directory=args.input_dir,
